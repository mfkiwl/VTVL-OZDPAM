...
const float Accel_Z_corrector = 1 / 14418.0;

float aRes_MPU6050, gRes_MPU6050;      // scale resolutions per LSB for the sensors

int16_t accelCount_MPU6050[3];  // Stores the 16-bit signed accelerometer sensor output
int16_t gyroCount_MPU6050[3];   // Stores the 16-bit signed gyro sensor output

float gyroBias_MPU6050[3] = {0, 0, 0}, accelBias_MPU6050[3] = {0, 0, 0};      // Bias corrections for gyro and accelerometer

float SelfTest_MPU6050[6];    // holds results of gyro and accelerometer self test

float ax_MPU6050, ay_MPU6050, az_MPU6050, gx_MPU6050, gy_MPU6050, gz_MPU6050, mx_MPU6050, my_MPU6050, mz_MPU6050; // variables to hold latest sensor data values
...
void calibrateMPU6050(I2C_HandleTypeDef *I2Cx, MPU6050_t*DataStruct, float * dest1, float * dest2){
  ...
  uint8_t writeData;
	...
	if(SerialDebugB){
		float gyroBias_MPU6050X = (float) gyro_bias[0]/(float) gyrosensitivity;
		float gyroBias_MPU6050Y = (float) gyro_bias[1]/(float) gyrosensitivity;
		float gyroBias_MPU6050Z = (float) gyro_bias[2]/(float) gyrosensitivity;
	...
	if(SerialDebugB){
		float accelBias_MPU6050X = (float) accel_bias[0]/(float) accelsensitivity;
		float accelBias_MPU6050Y = (float) accel_bias[1]/(float) accelsensitivity;
		float accelBias_MPU6050Z = (float) accel_bias[2]/(float) accelsensitivity;
